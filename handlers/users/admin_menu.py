...
def get_about_bot():
    ...
    message = "<b>📰 ВСЯ ИНФОРАМЦИЯ О БОТЕ</b>\n" \
              f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
              f"<b>🔶 Пользователи: 🔶</b>\n" \
              f"🦣 Мамонтов в боте: <code>{len(show_users)}</code>\n" \
              f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
              f"<b>🔶 Счета в боте</b>\n" \
              f"💸 Продажи на сумму: <code>{show_profit_day}руб</code>\n" \
              f"💰 Заскамлено мамонтов 🦣: <code>{show_profit_all}руб</code>\n" \
              f"📕 Всего пополнений от мамонтов за 24 часа: <code>{show_buy_day}руб</code>\n" \
...
async def get_chat_id(message: types.Message, state: FSMContext):
    ...
    if len_items >= 1:
        ...
        await message.answer("<b>❗ Данный момент товар отсутствует</b>")
