...
async def show_search(message: types.Message, state: FSMContext):
    ...
    if len(get_categories) >= 1:
        ...
        await message.answer("<b>🔷 Выберите населенный пункт:</b>", reply_markup=get_kb)
    else:
...
# Обработка кнопки "ℹ️ Информация"
...
@dp.message_handler(text="↗️ Перейти на сайт", state="*")
...
async def open_category_for_buy_item(call: CallbackQuery, state: FSMContext):
    ...
    if len(get_positions) >= 1:
        await call.message.edit_text("<b>🔷 Выберите город:</b>",
                                     reply_markup=get_kb)
    else:
...
async def open_category_for_create_position(call: CallbackQuery, state: FSMContext):
    ...
    send_msg = f"<b>🛍 Информация о товаре</b>\n" \
               f"➖➖➖➖➖➖➖➖➖➖➖➖➖\n" \
               f"<b>🔷 Населеный пункт:</b> <code>{get_category[2]}</code>\n" \
               f"<b>🔷 Товар:</b> <code>{get_position[2]}</code>\n" \
               f"<b>🔷 Цена:</b> <code>{get_position[3]}руб</code>\n" \
...
async def buy_item_next_page_position(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(f"""🔗 Ваша реферальная ссылка - <code>t.me/manyshopp_bot?start=ref-{id}</code>

📤 Отправьте её вашим друзьям. Если Ваш друг зарегистрируется по ссылке и пополнит баланс, то вы получите 10 рублей на баланс!""",
